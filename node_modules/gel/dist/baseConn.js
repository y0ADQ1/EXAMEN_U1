...
class BaseRawConnection {
    ...
    _encodeParseParams(wb, query, outputFormat, expectedCardinality, state, capabilitiesFlags, options, language) {
    ...
    async _parse(language, query, outputFormat, expectedCardinality, state, capabilitiesFlags = exports.RESTRICTED_CAPABILITIES, options) {
        const wb = new buffer_1.WriteMessageBuffer();
        wb.beginMessage(chars.$P);
        this._encodeParseParams(wb, query, outputFormat, expectedCardinality, state, capabilitiesFlags, options, language);
        wb.endMessage();
        wb.writeSync();
        ...
        if (error !== null) {
            if (error instanceof errors.StateMismatchError) {
                return this._parse(language, query, outputFormat, expectedCardinality, state, capabilitiesFlags, options);
            }
            throw error;
        }
        return [
    ...
    async _executeFlow(language, query, args, outputFormat, expectedCardinality, state, inCodec, outCodec, result, capabilitiesFlags = exports.RESTRICTED_CAPABILITIES, options) {
        ...
        if (error != null) {
            if (error instanceof errors.StateMismatchError) {
                return this._executeFlow(language, query, args, outputFormat, expectedCardinality, state, inCodec, outCodec, result, capabilitiesFlags, options);
            }
            throw error;
        }
        return warnings;
    ...
    async fetch(query, args = null, outputFormat, expectedCardinality, state, privilegedMode = false, language = ifaces_1.Language.EDGEQL) {
        ...
        if ((!inCodec && args !== null) ||
            ...
            [card, inCodec, outCodec, _, _, _, warnings] = await this._parse(language, query, outputFormat, expectedCardinality, state, privilegedMode ? Capabilities.ALL : undefined);
            this._validateFetchCardinality(card, outputFormat, expectedCardinality);
        }
        try {
            warnings = await this._executeFlow(language, query, args, outputFormat, expectedCardinality, state, inCodec ?? codecs_1.NULL_CODEC, outCodec ?? codecs_1.NULL_CODEC, ret, privilegedMode ? Capabilities.ALL : undefined);
        }
        catch (e) {
